	...
	const onSubmit = async (data: ISearchByIdForm) => {
		...
		try {
			if (baseUrl && data !== null) {
				const response: AxiosResponse<IParkingById[]> = await SearchSpotsById(baseUrl, data);
				...
				<div className={styles.searchFormContainer}>
					<div className={styles.inputWrap}>
						...
						<Controller
							...
							defaultValue=""
							render={({ field }) => (
								<input className={styles.input} id="id" type={InputTypeList.Text} placeholder="userId" {...field} />
							)}
						/>
					</div>
